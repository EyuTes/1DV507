 ...
 * 13 jan 2017
 */
package eb223db_assign1.Exercise8;
...
public class FunctionPointers {
	...
	public static void main(String[] args) {
		...
		List<Integer> list = Arrays.asList(45, 3, 24, 16, 1, 1, 3, 8, 7, 6, 10,
				12, 17, 22, 30);
		...
		System.out
				.println("Square root: " + applyFunction(numbers, Math::sqrt));
		System.out.println("Power of two: "
				+ applyFunction(numbers, FunctionPointers::powerOfTwo));
	}

	// Prints all elements in the list where predicate evaluates to true
	public static void selectAndPrint(List<Integer> list,
			Predicate<Integer> predicate) {

		for (Integer intger : list) {
			if (predicate.test(intger)) {
				System.out.print(intger + " ");
			}
		}
	}

	// Returns a new list containing the numbers resulting from applying fx on
	// the input list numbers
	private static List<Double> applyFunction(List<Double> numbers,
